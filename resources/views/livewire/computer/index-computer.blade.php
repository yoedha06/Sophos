<div>
    <div class="content">
            ...
            <div class="max-w-screen-xl px-4 py-3 mx-auto">
                <div class="flex flex-col"> <!-- Changed from flex-row to flex-col -->
                    <ul class="flex flex-row font-medium mt-0 space-x-8 rtl:space-x-reverse text-sm">
                        <li>
                            <h1 class="text-3xl font-semibold">Endpoint Protection - Computers</h1>
                        </li>
                    </ul>
                    <ul class="flex flex-row font-medium mt-0 space-x-8 rtl:space-x-reverse text-sm">
                        <li>
                            <h5 class="text text-gray-300"><span class="text-blue-500">Overview </span> / <span
                                    ...
                                    class="text-gray-400">Computers</span></h5>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <div>
                    <div class="flex justify-between border-b items-center pb-2" style="margin-left: 20px;">
                        <div class="flex space-x-1">
                            <button class="px-2 py-1 text-xs text-gray-400  bg-gray-200 rounded cursor-not-allowed" disabled>
                                Manage Endpoint Software
                            </button>
                            <button class="px-2 py-1 text-xs text-gray-400 bg-gray-200 rounded cursor-not-allowed" disabled>
                                Turn on tamper protection
                            </button>
                            <button class="px-2 py-1 font-semibold text-xs text-blue-600 bg-gray-100 border border-gray-200 rounded hover:bg-gray-200">
                        ...
                        <div class="flex space-x-1 px-5">
                            <button class="text-xs text-blue-600 hover:underline">Export to CSV</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <table class="border-b ml-4 text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <tbody class="text-xs text-gray-700 uppercase dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    <th scope="col" class="px-2">
                        <select id="small"
                            ...
                            <option selected>All Computers</option>
                            <option>Mac Computers</option>
                            <option>Windows Computers</option>
                        </select>
                    </th>
                    <th scope="col" class="px-2">
                        <select id="small"
                            class="block w-full p-2 mb-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option selected>Show all computers</option>
                            <option>Computers with a medium or bad status</option>
                            <option>Computers with a bad status</option>
                            <option>Computers with tamper protection off</option>
                        </select>
                    </th>
                    <th scope="col" class="px-2">
                        <select id="small"
                            class="block w-full p-2 mb-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option selected>Any protection types</option>
                            <option>Full Sophos protection</option>
                            <option>Some Sophos protection missing</option>
                            <option>Sophos XDR Sensor</option>
                            <option>No Sophos protection</option>
                            <option>Computers with Device Encryption</option>
                            <option>Computers withot Device Encryption</option>
                        </select>
                    </th>
                    <th scope="col" class="px-2">
                        <select id="small"
                            class="block w-full p-2 mb-2 text-sm border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option selected>Recently online</option>
                            <option>No online recently</option>
                            <option>All</option>
                        </select>
                    </th>
                    <th scope="col" class="px-2">
        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    @include('livewire.includes.sort-table-th', [
                        'hostname' => 'hostname',
                        'displayName' => 'Name'
                    ])
                    @include('livewire.includes.sort-table-th', [
                    ...
                    @include('livewire.includes.sort-table-th', [
                        'hostname' => "tamper_protection_enabled",
                ...
                @foreach ($computers as $computer)
                    <tr
                        ...
                        <td scope="row" class="px-18 pr-2 py-10 font-medium text-gray-900 whitespace-nowrap dark:text-white flex items-center">
                            ...
                            @else
                                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.--><path fill="#ff0000" d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zm0-384c13.3 0 24 10.7 24 24l0 112c0 13.3-10.7 24-24 24s-24-10.7-24-24l0-112c0-13.3 10.7-24 24-24zM224 352a32 32 0 1 1 64 0 32 32 0 1 1 -64 0z"/></svg>
                            @endif
                            <a wire:navigate href="/details/{{ $computer->id_computer }}" class=" text-blue-600">
                                {{ $computer->hostname }}
                            </a>                            
                        </td>                        
                        ...
                        <td class="px-3 py-5">
                            <i class="fas fa-check"></i>&nbsp;{{ $computer->assigned_products[2]['code'] }}
                        </td>
                        <th scope="row"
                        ...
                        <td class="px-6 py-4">
                            {{ $computer->created_at->format('M d, Y h:i A') }}
                        </td>
                        <td class="px-6 py-4">
                            {{ $computer->group['name'] ?? null }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            @if ($computer->tamper_protection_enabled == true)
                                on
                            @else
                                off
                            @endif
        ...
        {{-- <footer class="fixed bottom-0 w-full bg-gray-50">
